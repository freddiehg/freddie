...
struct Agent {
    /// The job's name, keyed to the same [`APP`] as the lock and the log directory, so a fork that
    /// renames the app gets its own job rather than fighting this one over a shared label.
    label: String,

    /// `daemon`, never the bare binary: `mercury` spawns a detached daemon and exits, which would
...
/// The launchd job's name, keyed to the same [`APP`] as the lock and the log directory, so a fork
...
#[cfg(test)]
mod tests {
    use super::{Agent, label, record_level, show};
    use std::path::Path;
    use tracing::Level;
