...
- freddie is the framework: the event loop, effects-as-data, and the glue over laserbeam and bind.
- laserbeam is a library within freddie: the typed mutable path (resolve, `into_parent`, `get_root`).
...
- At any moment there is exactly one active path from the root down to the active leaf. That is the laserbeam resolve target.
...
## Accumulating the active binding set (a runtime computation via laserbeam)
...
1. Resolve to the active leaf with laserbeam.
...
3. When an event fires, use laserbeam to find the single handler for it and execute it.
...
A future cannot hold a laserbeam path across its await, because the mutable borrow cannot outlive a dispatch. So the future carries plain data or a key, does its I/O off the dispatch thread, and when it resolves it re-enters as a completion event: the loop re-resolves from the current root and applies the result through the same synchronous path. See the serializability open question below.
...
When an event is handled, laserbeam finds the first and only handler for it, and runs it. The handler receives:

- the event, and
- a laserbeam path to the node the binding is on.
...
Because each path's parent chain terminates at `&mut Root`, `get_root` is a fixed walk up the chain, the same shape laserbeam already uses for `into_parent`. The laserbeam derive knows each node's parent, so it can generate the `GetRoot` impl per path type the same way it generates `Resolve`. Whether this lives in laserbeam or freddie is an open question below.
...
6. Relationship to the missing laserbeam features. Layers as an enum and upward traversal are supported today; if a layer ever holds a list of sub-states (a `Vec` of children) selected by state, that is the state-controlled-children feature scoped in `laserbeam-state-controlled-children.md`.
7. Where `GetRoot` lives. It is a generalization of `into_parent` to "walk all the way up," so laserbeam is the natural home, and the derive can emit it from the parent chain alongside `Resolve`. But layer-switching is freddie semantics. Decide whether laserbeam exposes the generic `get_root` and freddie builds layer-switching on it, or freddie owns the whole trait.
...
12. Incremental reaccumulation (perf). laserbeam could record the rootmost node mutably accessed during a dispatch. Since nothing above that node changed, reaccumulation only needs to redo the path at and below it and reuse the rest of the accumulated set. Not v1; a perf optimization to keep in mind.
