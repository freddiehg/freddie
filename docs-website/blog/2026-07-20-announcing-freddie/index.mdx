...
`mercury` boots into the typing layer, where every keystroke passes through. Typing `jk` takes you home. From there, `n` is nav, `i` is in-app, `s` is per-site, `r` is resize, and `o` shows you an overlay of the layer's keymap.
