...
Given a mutable reference to a deeply nested struct, laserbeam produces a `Path` struct capable of two things: providing a mutable reference to the active leaf node, and the ability to produce a `Path` that is one layer closer to the root.
...
`#[derive(Laserbeam)]` implements `Resolve` for each node, so `resolve` on the root walks down the active variants to the one live leaf, returning a `Path`. `get_mut` borrows the focused node; `into_parent` consumes the path and hands you a path focused on the parent one level up.
...
laserbeam seeks to provide a better DevEx for the accurate representation.
...
laserbeam is the mutable analogue of Isograph's [`resolve_position`](https://github.com/isographlabs/isograph/tree/main/crates/resolve_position). They should be combined.
