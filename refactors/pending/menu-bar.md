...
- `Command`'s definition: is it a distinct enum mapped to `MercuryEvent` in the glue, or does a menu item carry a `MercuryEvent` directly. Direct is less indirection; a separate `Command` keeps clickable intents from having to be expressible as synthetic events if some menu action is not.
