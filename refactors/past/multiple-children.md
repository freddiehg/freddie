...
3. bind_macro: `find_child`'s at-most-one restriction lifts (every `#[child]` field, declaration order); a place node may carry both fields and `#[derived_children]`. Every place node emits the uniform body above — the standing init plus one `descend` block per child edge, field blocks then derived blocks; a leaf's emission is unchanged (the init is today's leaf body), and a single-child node's emission changes shape, not behavior. Two fields of the same type are legal (the two-keyboards case): `HasPath` is per-type, so both children share one path type and one `parent_path` declaration, and only the projections distinguish them, which dispatch is fine with. A routed or generic child at a multi-child node is rejected with a derive error, mirroring `reject_routed_generic`; the messages and the work that lifts them are `multiple-children-generic-routed.md`. The derived-level rejection of `#[child]` stays. Accumulate, under `check`, is not extended: a multi-child node's `AccumulateTriggers` impl is emitted with no child bounds and a body of `Err(BindError::MultiChildNode)`, so check builds compile everywhere and the check errors at the call for any tree containing a branch point. The check's eventual rework accumulates into a `Vec`, where a duplicate is legal because claim order is the semantics; that is recorded, not built here.
4. figaro: flatten the typing layer as written above.

## Open questions

1. A recorded limit, with no known consumer: the schedule never says which branch left. The join keeps "did any leave reach this node" and folds the attribution away, so a branch-point item keyed on "branch i left" cannot be written. Nothing needs one: per-branch machinery (a deadline, an overlay) lives at the branch's own top node, where its own one bit is enough and every event still reaches it; guards cancel on drop when a subtree dies; and a replacement of a branch flows through a transition method on the branch point, which knows its target and emits the cleanup, as `set_layer` does. A consumer, if one ever appears, would force outcomes onto the ascend side and change branch-point handler signatures — which is why the limit is written down.

2. The derive's other edges under a branch: a routed (multi-parent) child (`#[child(route = .., up = ..)]`, whose fold matches the consumer's `Up` enum) and a generic child (the `for<'q>` path-equality bound from `generic-doll-nodes.md`) at a branch point are unexamined; `multiple-children-generic-routed.md` records the expansions to verify and the mocks that lift change 3's rejection. Neither occurs in the typing case: the flattening removes typing's generic children rather than adding any.
