# Laserbeam — design and implementation

Mutable counterpart to isograph's `resolve_position`. You put `#[derive(Laserbeam)]` on your state types and call `<Root as Resolve>::resolve(&mut root)`; it returns a typed path to the single active leaf, which mutates the leaf and walks up to its ancestors, one live `&mut` at a time. No `Rc`, no `RefCell`, no `unsafe`.

Two crates in the freddie workspace: `laserbeam` (the runtime, plus the re-exported derive) and `laserbeam_macro` (the derive). They depend on nothing else in the workspace. Both are implemented; the worked example below is `crates/laserbeam/tests/derived.rs`, and the hand-written shape it generates is `crates/laserbeam/tests/tree.rs`.
...
`#[derive(Laserbeam)]` reads exactly one role attribute, so a contradictory combination cannot be written:

- `#[laserbeam_root(resolved = R)]` on the root node.
- `#[laserbeam(path = P, resolved = R)]` on a non-root node, where `P` is the node's own path type and `R` is the shared resolved enum.
- `#[laserbeam_path(node = N)]` on a multi-parent node's path enum, where `N` is the node that enum is the path for.
...
This enum form is forced by the orphan rule. The earlier plan used isograph's single-path-type shape, `type TitlePath = Path<Title, TitleParent>`, and built it with a `From` impl. That cannot work once `Path` lives in the `laserbeam` crate: `impl From<..> for Path<Title, ..>` is an impl of a foreign trait for a foreign type (only the type arguments are local), which the orphan rule rejects (E0117). Making the path a local enum makes its `From` impls legal, since the target type is local. Single-parent nodes never hit this, because nothing converts into their path. `crates/laserbeam/tests/tree.rs` is the cross-crate proof.
...
You declare the types, because your own code references them: the state types, the path types (a `Path<Node, ParentPath>` alias for one parent, a `#[laserbeam_path]` enum for several), and the `Resolved` enum.

The derive emits impls only:

- For a node (`#[laserbeam_root]` / `#[laserbeam]`): `impl Resolve`. Its `resolve` body, for a struct, is `<Child as Resolve>::resolve(Path::from_fn(path, edge).into())`, where `edge` is the `#[resolve_into]` field projection; for a leaf it is `Resolved::<Self>(path)`; for an enum it matches the active variant and descends. The root's `Path` is `&mut Self` and its descent dereferences it; a non-root node re-derives through `path.get_mut()`.
- For a path enum (`#[laserbeam_path]`): a `From<variant payload>` per variant (the trivial wrap), and a `get_mut` that dispatches to the active variant.
...
- `tests/tree.rs`: the example with hand-written impls, in a crate separate from `laserbeam` (the orphan-rule proof). Resolves through each branch, mutates the leaf, walks up to mutate an ancestor, reads an ancestor through `parent`.
- `tests/derived.rs`: the same tree via `#[derive(Laserbeam)]`, plus a resolve / mutate / re-resolve round trip.
...
`.github/workflows/ci.yml` runs `cargo fmt`, `cargo clippy --all-targets --all-features -D warnings`, and `cargo test --all --all-features`, on a pinned 1.96.0 toolchain so clippy and trybuild match locally. A push to `master` publishes `0.0.1-main-<7-char sha>` of every crate; a `v*` tag publishes that version. The version step rewrites both the workspace version and the path-dependency version specifiers, so the published `laserbeam` depends on the matching `laserbeam_macro`.
