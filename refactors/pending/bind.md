...
`#[bind(trigger, handler)]` on a node (a `Rayban`-derived struct or enum), repeatable:
...
- `trigger`: an expression evaluating to a value that lifts into the consumer's `Trigger` enum via `Into` (`derive_more::From`). The derive wraps it with `.into()` when building the node's binding set, so `Keyboard::new('g')` and a `Foreground::new("Chrome")` both land as `Trigger`.
- `handler`: a function (signature below).
- Several `#[bind]` on one node accumulate into that node's binding set.

## The Trigger type (consumer-provided)
...
`bind` is generic over it with `T: Hash + Eq + Clone`. `bind` never constructs or matches `Trigger` variants itself; it treats `T` as an opaque hashable key. Matching variants to actually register and deregister is the consumer's registrar (below).

## What the derive emits

Per node, the derive emits that node's bindings: its `(trigger, handler)` pairs, where each handler is callable later with the event and a rayban path to this node. The exact trait shape is open (see open questions), but the contract is that a node yields its own bindings and nothing about its position in the tree; position comes from rayban at accumulation time.
...
3. Coalesce into one map keyed by `T`.

Output: the active binding map `HashMap<T, Thunk>`, whose key set is the active set `HashSet<T>` used for registration.
...
v1 is static and non-clobberable: a trigger bound at two levels of the active path is a collision and an error raised here, during accumulation, naming both bindings. The fuller space (leafward-clobbers-rootward precedence, clobberable bindings, dynamic fall-through) is in `freddie-dispatch-precedence.md` and is not in v1.
...
`bind` computes the active trigger set per state. Across a state change it diffs old vs new (set subtraction, since `T: Hash + Eq`) into `to_register` and `to_deregister`. `bind` does not touch the OS; it produces the delta and the consumer routes each trigger by variant to its backend through a registrar:

```rust
trait Registrar<T> {
    fn register(&mut self, triggers: &HashSet<T>);
    fn deregister(&mut self, triggers: &HashSet<T>);
}
```

The registrar owns all registration state (taps, watchers, per-trigger OS handles in a map keyed by `T`); trigger values stay pure data and hold no references to it. Whether `bind` calls the registrar itself or just returns the delta to the loop is open.
...
- The consumer brings: the `Rayban`-derived state types, the `Trigger` enum, the handlers, the `Event` and effect types, the `Registrar` impl, and the event stream that yields `(Trigger, Event)`.
...
2. Genericity: parameterize `bind` over `(T, Event, HandlerOutput)` via one trait the consumer implements, vs free type params on each entry point.
