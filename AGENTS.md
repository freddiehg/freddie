...
- The line between the two shapes: a value rides its event when the source holds it or the callback produces it synchronously, because then no gap exists between knowing-it-changed and knowing-it. A cross-process read is asynchronous, so its gap is modeled: the fact event mints a `freddie_sync` token pair in the model, the entry goes `Pending` with one half, the read effect carries the other, and the value event brings it home to `commit`. Speed is never the test; synchrony with the announcement is.
