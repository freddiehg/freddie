...
///
/// A daemon reads its directives from `LOG_LEVEL`, a `tracing_subscriber` filter string,
/// so `info` and `mercury=debug,bind=warn` are both accepted. One that does not parse
/// falls back to [`DEFAULT_LOG_LEVEL`] and says so, since the alternative is a run with
/// no logging.
pub fn init(terminal: &Terminal) -> PathBuf {
    let dir = log_dir();
