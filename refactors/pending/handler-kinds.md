...
Down. Each node builds its child path (`from_fn`) and recurses. A pre/post node runs its `pre`s here: each matching `pre` pushes its now-effects onto the batch (the `A_pre`, `B_pre` of the order) and binds its `T` into `opt_i: Option<T>` (`Some` iff the trigger matched), stored on the child path beside the post. No exclusive handler fires on the way down.
...
after (`T` is `pre`'s return, inferred; the post takes the same `Node<&mut Path>` the pre did, not a bare `&mut N`, so it can `ascend` as well as `get_mut`):

```rust
pub struct PathMut<'a, N, P, T> {
    /* projection to N, parent P */
    post: fn(T, Node<&mut P, N>, Nested) -> Vec<Effect>,   // `no_post` for a node with none
...
impl<'a, N, P, T> PathMut<'a, N, P, T> {
    ...
    pub fn into_parent(mut self, nested: Nested, sink: &mut Vec<Effect>) -> P {
        if let Some(t) = self.held.take() {
            // reborrow the level as a `Node` so the post gets the pre's `&mut Path` access
            Extend::extend(sink, (self.post)(t, self.node(), nested));
        }
        self.parent
    }
}
...
fn no_post<N, P, T>(_: T, _: Node<&mut P, N>, _: Nested) -> Vec<Effect> { Vec::new() }
```

The post runs INSIDE `into_parent`, so it runs on both drivers through the one funnel: the miss-unwind's `Continue` (`Nested::Missed`) and `complete`'s climb on the fire side (`Nested::Handled`). That is the whole reason `complete` exists — it is `into_parent` in a loop, so a winner firing deep runs every post above it, on the live path, before `set_layer`.
...
- feature (this doc): `PathMut` gains `post: fn(T, Node<&mut P, N>, Nested) -> Vec<Effect>`, `held: Option<T>`, and the `T` param; `into_parent` runs the post through the seam, and `ascend`/`complete` gather the crossed posts. `pre` becomes `-> (T, Vec<Effect>)`. `bind_macro` gains `#[pre_post]` / `#[pre]` / `#[post]` — the descent-time pre binding `opt_i` and pushing now-effects, `from_fn` storing the user's `post`.
