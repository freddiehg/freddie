...
The rule cuts the other way as often. `held` sits at the root, because entering and leaving a passthrough layer reads it to synchronize the app's view of the modifier keys, so it has to outlive any one layer. The `jk` sequence answers the same test the other way: it lives on `TypingLayer`, built fresh on entry and dropped with the layer, so a half-typed run never crosses a boundary. The test is which level uses the state, not which level is convenient to hang it on.
